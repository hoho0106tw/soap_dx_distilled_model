...
自動判斷單一主要診斷（Primary Diagnosis），以提升臨床資料判讀效率。  

本模型基於約 GPT 20B 等級架構，使用約 7000 筆臨床資料進行訓練，  
並經過約五天的資料挑選、清洗與整理，提升資料品質與一致性。  

在模型開發過程中，透過約兩天的模型觀察與生成行為分析，  
針對輸出形式與推論策略進行優化，最終於一週內完成整體模型雛型的建構。  

同時結合機器學習與統計方法進行資料優化，  
使模型在分類任務上達到穩定且良好的表現，整體準確率可達 **90% 以上**。  

此外，透過輸出控制與推論優化，模型僅需生成約 **10 tokens** 即可完成診斷預測，  
相較於原始模型需較長輸出，大幅提升推論效率並降低計算成本。  
---
